...
export async function bin (task, opts) {
  await task.source(opts.src || 'src/bin/*').babel().target('dist/bin', {
      mode: '0755'
  })
...
export async function server (task, opts) {
  await task.source(opts.src || 'src/server/**/*.js').babel().target('dist/server')
  notify('Compiled server files')
}

export async function client (task, opts) {
  await task.source(opts.src || 'src/client/**/*.js').babel().target('dist/client')
  notify('Compiled client files')
}

export async function copy (task) {
  await task.source('src/pages/**/*.js').target('dist/pages')
}
